...
dir = sys.argv[7]
name = sys.argv[5]
bpy.ops.import_scene.fbx(filepath=f'{dir}{name}')
frame = name.split("_")
file_name = frame.pop(0)
frame[len(frame)- 1 ] = frame[len(frame)- 1].split(".")[0]
...
ob_v = bpy.data.objects['root']
bpy.context.view_layer.objects.active = ob_v
...
if selection:
    ...
    for i in keys:
        ...
        bpy.ops.export_scene.gltf(export_format='GLTF_EMBEDDED',filepath = f"{path}/{file_name}_{i}",export_current_frame = True)
else:
